...
2. Update the active session state file for an active task in `workflow: patch`, `phase: scope`.
...
2. Update the active session state file to `phase: patch`.
...
2. Update the active session state file to `phase: sanity`.
...
Reload the active session state file and the active workflow:
